...
class VacuumPlanning(Problem):
    ...
    def path_cost(self, curNode, state1, action, state2):
        ...
        if curNode.action == None:
            action1 = None
        else:
            action1 = curNode.action
        if self.env.costFunc == 'Step':
            cost = curNode.path_cost+1
        elif self.env.costFunc == 'StepTurn':
            cost = curNode.path_cost + self.computeTurnCost(action1,action)
        elif self.env.costFunc == 'StayLeft':
            if action == 'LEFT':
                cost = curNode.path_cost + 0.5
            else:
                cost = curNode.path_cost + 1
            ...
            cost = 0
            print("Error: No such cost function.\n")
        return cost
    ...
    def computeTurnCost(self, action1, action):
        ...
        hor = ['UP','DOWN']
        ver = ['RIGHT','LEFT']
        if (action1 in hor and action in ver) or (action1 in ver and action in hor ):
            return 0.5
        else:
            return 1
    ...
    def h(self, node):
        ...
        heur = 0
        if self.env.distanceFunc == 'Manhattan':
...
if __name__ == "__main__":
    ...
    env.distanceFunc = args['heuristic']
    win.mainloop()
