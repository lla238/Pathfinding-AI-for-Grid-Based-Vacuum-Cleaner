...
def breadth_first_graph_search(problem):
    ...
    if problem.goal_test(node.state):
        return node, None
    frontNode = deque([node])
    explored = set()

    while frontNode:
        node = frontNode.popleft()
        explored.add(tuple(node.state))

        for child in node.expand(problem):
            if child not in frontNode and tuple(child.state) not in explored:
                if problem.goal_test(child.state):
                    return child, explored
                frontNode.append(child)
...
def best_first_graph_search(problem, f=None):
    ...
    while frontNode:
        dist,node = heapq.heappop(frontNode)
        explored.add(tuple(node.state))
        ...
        for child in node.expand(problem):
            if child not in frontNode and tuple(child.state) not in explored:
                if problem.goal_test(child.state):
                    return child, explored
                heapq.heappush(frontNode, (f(child), child))
